...
UNIT = "kWh"  # sensor unit: "MWh" or "kWh"
...
def load_and_prepare(filepath: str) -> pd.DataFrame:
    ...
    if n_dropped:
        print(
            f"  ⚠  Dropped {n_dropped} rows (negative diffs or spikes > {SPIKE_FILTER} kWh)"
        )
    else:
...
def print_monthly(df: pd.DataFrame):
    ...
    print("=" * 54)
    monthly = (
        df.groupby(["month", "period"])["consumption_kwh"].sum().unstack(fill_value=0)
    )
    ...
    for m, row in monthly.iterrows():
        print(
            f"  {str(m):<10} {row['Off-Peak']:>10.1f} {row['Peak']:>10.1f} "
...
def print_daily_averages(df: pd.DataFrame):
    ...
    print("=" * 54)
    daily = (
        df.groupby(["date", "period"])["consumption_kwh"].sum().unstack(fill_value=0)
    )
    for col in ["Peak", "Off-Peak"]:
    ...
    for m, row in avg.iterrows():
        print(
            f"  {str(m):<10} {row['Off-Peak']:>10.2f} {row['Peak']:>10.2f} {row['Total']:>10.2f}"
        )
    print()
    return daily
...
def write_excel(df: pd.DataFrame, monthly, daily, output_path: str):
    ...
    try:
        ...
        print(
            "  ⚠  openpyxl not installed — skipping Excel output (pip install openpyxl)"
        )
        return
